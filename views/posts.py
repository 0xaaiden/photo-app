...
from my_decorators import is_valid_int, is_valid_int_delete, handle_db_insert_error
def get_path():
...
class PostListEndpoint(Resource):
    ...
    def get(self):
        ...
        print(authorized)
        Posts = Post.query.filter(Post.user_id.in_(authorized))
        limit = request.args.get("limit")
        if limit:
            try:
                limit = int(limit)
            except:
                return Response(json.dumps({"message": "Limit must be an int between 1 and 50"}), mimetype="application/json", status=400)	
        ...
        data = Posts.order_by(Post.pub_date.desc()).limit(limit)
        data = [
